...
async function loadAndConfigureAlloy(instanceName, webSDKConfig) {
  await import('./libraries/alloy.min.js');
  try {
...
function onDecoratedElement(fn) {
  ...
  if (
    document.querySelector('[data-block-status="loaded"],[data-section-status="loaded"]')
  ) {
    fn();
  }
...
async function loadAndConfigureDataLayer() {
  await import('./libraries/acdl.min.js');
  if (config.analytics) {
...
export async function updateUserConsent(consent) {
  ...
  const fn = () =>
    window[config.alloyInstanceName]('setConsent', {
      consent: [
        {
          standard: 'Adobe',
          version: '2.0',
...
async function applyPropositions(instanceName) {
  ...
  let propositions = window
    ...
    .filter((p) =>
      p.items.some((i) => i.schema === 'https://ns.adobe.com/personalization/dom-action'),
    );
  onDecoratedElement(async () => {
    if (!propositions.length) {
      return;
    }
    const appliedPropositions = await window[instanceName]('applyPropositions', {
      propositions,
    });
...
export function initRumTracking(sampleRUM, options = {}) {
  ...
  if (sampleRUM.always) {
    track = (ev, cb) =>
      sampleRUM.always.on(ev, (data) => {
        debug('rum', ev, data);
        cb(data);
      });
  } else {
    track = (ev, cb) =>
      document.addEventListener('rum', (data) => {
        debug('rum', ev, data);
        cb(data);
      });
  }
...
export async function martechDelayed() {
  ...
  return Promise.all(launchUrls.map((url) => import(url))).catch((err) =>
    handleRejectedPromise(new Error(err)),
  );
}
